...
public class MathsScript : MonoBehaviour //class for calculating the math used in the game
{

    float[] Matrix_multiplication(Matrix pMatrix, float[] Arrey) //matrix multiplication by array
    {
        ...
        for (int row = 0; row < size; row++)
        {
            ...
            {
                param[col] += (float)pMatrix[row, col] * Arrey[row];
            }
        }
    ...
    float MatrixItemCreation(float[] arrey, int power)// matrix element creation
    {
        float newItem = 0;

        for (int i = 0; i < arrey.Length; i++)
        {
            newItem = newItem + (float)Math.Pow((double)arrey[i], power);
        }

        return newItem;
    }

    public float[] MNK(int power, float[] x, float[] y) //matrix creation  
    {
        int sizeArrey = x.Length;
        int powerItem = power * 2;



        Matrix Mat = new Matrix(power + 1, power + 1);
        float[] factor = new float[power + 1];
        for (int i = 0; i < power + 1; i++)
        {
            for (int j = 0; j < power + 1; j++)
            {
                ...
                {
                    Mat[i, j] = (double)MatrixItemCreation(x, (powerItem - (i + j)));
                }

            }
            factor[i] = MatrixItemCreation(x, power - i) * MatrixItemCreation(y, 1);

        }


        Matrix IM = Mat.CreateInvertibleMatrix();


        if (IM != null)
        {
            return Matrix_multiplication(IM, factor);
        }
        return null;
    }

    public float GetYfromX(float X, ArrayList Arguments)// It computes y = F ((a1x ^ n) + (a2 * x ^ n-1) + ... + an), where n is the number of arguments in the Arguments array 
    {
        float Y = 0;
        int n = Arguments.Count - 1;
        float Arg;
        float xn;
        for (int i = 0; i < n; i++)
        {
            Arg = (float)Arguments[i];
            xn = (float)Math.Pow((double)X, (n - i));
            Y = Y + Arg * xn;
        }
        Y = Y + (float)Arguments[n];
        return Y;
    }

    float Li(int i, float[] XArrey, float Xcurrent) // product Li in the polynomial Lagrange
    {
        float l = 1;
        int size = XArrey.Length;
        float Xij, Xcj;
        for (int j = 0; j < size; j++)
        {
            Xij = (XArrey[i] - XArrey[j]);
            Xcj = (Xcurrent - XArrey[j]);
            if ((i != j) && (Xij != 0.0f))
            {
                l = l * Xcj / Xij;
            }
    ...
    public float LagrangePolynomial(float[] XArrey, float[] YArrey, float Xcurrent) //  Polynomial Lagrange
    {
        float Y = 0;
        int size = XArrey.Length;
        for (int i = 0; i < size; i++)
        {
            Y = Y + YArrey[i] * Li(i, XArrey, Xcurrent);
        }
        return Y;
    }
